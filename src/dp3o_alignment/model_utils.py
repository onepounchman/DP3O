...
def get_tokenizer(model_args: ModelArguments, data_args: DataArguments) -> PreTrainedTokenizer:
    ...
    if data_args.chat_template is not None:
        ...
        tokenizer.chat_template = getattr(tokenizer, "default_chat_template", None) or DEFAULT_CHAT_TEMPLATE

    # Llama-3 chat turns end in EOT, not the base model's end-of-text token.
    # Persist this EOS choice so SFT, TRL's appended EOS, and generation agree.
    if "<|eot_id|>" in tokenizer.chat_template and "<|eot_id|>" in tokenizer.get_vocab():
