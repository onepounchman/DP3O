...
from .type_utils import coerce_cli_value
...
class H4ArgumentParser(HfArgumentParser):
    @staticmethod
    def _coerce_cli_value(value: str, annotation: Any) -> Any:
        """Coerce a ``key=value`` override using its dataclass annotation."""
        return coerce_cli_value(value, annotation)
    ...
    def parse_yaml_and_args(self, yaml_arg: str, other_args: Optional[List[str]] = None) -> List[dataclass]:
        ...
        parsed_overrides = {}
        for raw_arg in other_args or []:
            if "=" not in raw_arg:
                raise ValueError(f"YAML overrides must use key=value syntax, got {raw_arg!r}")
            key, value = raw_arg.split("=", 1)
            key = key.strip("-")
            if key in parsed_overrides:
                raise ValueError(f"Duplicate argument provided: {key}")
            parsed_overrides[key] = value
        other_args = parsed_overrides
        used_args = {}
        ...
        for data_yaml, data_class in zip(arg_list, self.dataclass_types):
            ...
            for arg, val in other_args.items():
                ...
                if arg in keys:
                    ...
                    inputs[arg] = self._coerce_cli_value(val, base_type)
